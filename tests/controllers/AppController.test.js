...
describe('TEST: AppController class', () => {
  ...
  describe('TEST: GET: /status', () => {
    it('TEST: Services are online', function (done) {
      request.get('/status')
        .expect(200)
        .end((err, res) => {
          if (err) {
  ...
  describe('TEST: GET: /stats', () => {
    it('TEST: Correct statistics about db collections', function (done) {
      request.get('/stats')
        .expect(200)
        .end((err, res) => {
          if (err) {
    ...
    it('TEST: Correct statistics about db collections [alt]', function (done) {
      this.timeout(10000);
      Promise.all([dbClient.usersCollection(), dbClient.filesCollection()])
        .then(([usersCollection, filesCollection]) => {
          Promise.all([
