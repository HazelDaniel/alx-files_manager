...
describe('TEST: AuthController', () => {
  ...
  describe('TEST: GET: /connect', () => {
    it('TEST: Fails with no "Authorization" header field', function (done) {
      this.timeout(5000);
      request.get('/connect')
        .expect(401)
        .end((err, res) => {
    ...
    it('TEST: Fails with a valid email and wrong password', function (done) {
      this.timeout(5000);
      request.get('/connect')
    ...
    it('TEST: Fails with an invalid email and valid password', function (done) {
      this.timeout(5000);
      request.get('/connect')
    ...
    it('TEST: Succeeds for an existing user', function (done) {
      this.timeout(5000);
      request.get('/connect')
  ...
  describe('TEST: GET: /disconnect', () => {
    ...
    it('TEST: Succeeds with a valid "X-Token" field', function (done) {
      request.get('/disconnect')
        .set('X-Token', token)
        .expect(204)
        .end((err, res) => {
