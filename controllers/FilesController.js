...
const isValidId = (id) => {
  ...
  while (i < size) {
    ...
    if (!asciiCharRanges.some((range) => code >= range[0] && code <= range[1])) {
      return false;
    }
    i += 1;
  }
...
export default class FilesController {
  ...
  static async getShow(req, res) {
    const { user } = req;
    const paramsId = req.params ? req.params.id : NULL_ID;
    const userId = user._id.toString();
    const file = await (await dbClient.filesCollection())
      .findOne({
        _id: new mongoDBCore.BSON.ObjectId(isValidId(paramsId) ? paramsId : NULL_ID),
  ...
  static async getIndex(req, res) {
    ...
    const page = /[0-9]+/.test((req.query.page || '').toString())
